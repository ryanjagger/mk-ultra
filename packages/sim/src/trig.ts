/**
 * Deterministic trig via a committed quarter-wave lookup table + integer lerp.
 *
 * Angles are "brads": a full turn is 65536, so angles live in a uint16 and
 * wrap for free with `& 0xffff`. Angle 0 points along +x, increasing CCW
 * (toward +y), matching dir = (cos, sin).
 *
 * The table is Q16.16 sin(i/256 * PI/2) for i in 0..256 (plus a duplicated
 * endpoint so the lerp never indexes past the end). It was generated ONCE by
 * scripts/gen-trig-table.mjs and committed as literals — Math.sin is never
 * called at runtime, so cross-engine determinism is guaranteed.
 */
...
/** sin of an angle in brads -> Q16.16 in [-1, 1]. */
export function sinB(angle: number): Fx {
  const a = angle & 0xffff;
  const quad = a >> 14; // 0..3
  const idx = a & 16383;
  const t = (quad & 1) !== 0 ? 16384 - idx : idx; // fold to rising quarter
  const hi = t >> 6;
  const frac = t & 63;
  const s0 = SIN_TABLE[hi]!;
  const s1 = SIN_TABLE[hi + 1]!;
  const v = (s0 + (((s1 - s0) * frac) >> 6)) | 0;
  return quad >= 2 ? -v | 0 : v;
}

/** cos of an angle in brads -> Q16.16 in [-1, 1]. */
export function cosB(angle: number): Fx {
  return sinB(angle + BRAD_QUARTER);
}
